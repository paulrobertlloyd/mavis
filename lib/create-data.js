...
for (const [urn, patients] of Object.entries(schools)) {
  ...
  for (const campaign of campaigns) {
    ...
    session = Session.generate(urn, cohort, campaign)
    ...
    for (const patient of patientsInCampaign) {
      ...
      if (hasReplies) {
        const replyCount = faker.helpers.rangeToNumber({ min: 0, max: 2 })
        for (const _index in [...range(0, replyCount)]) {
          const reply = Reply.generate(campaign, session, patient)
          patient.attachReply(reply)
        }
      }
    }
  }
