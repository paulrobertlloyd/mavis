...
export class Session {
  ...
  static generate(urn, campaign, user) {
    ...
    // 10% of sessions take place today
    const today = new Date()
    const isToday = faker.helpers.maybe(() => true, { probability: 0.1 })
    let date = isToday
      ? today
      : addDays(created, faker.number.int({ min: 70, max: 100 }))
    ...
    return new Session({
      ...
      status: isToday ? SessionStatus.Active : SessionStatus.Planned,
